...
description: "Build polished visual web artifacts for the compose-yml frontend (React + Vite + Ant Design). Use when the user asks for pages, dashboards, prototypes, slide decks, animations, UI mockups, or data visualizations targeting the browser. Triggers on requests involving `frontend/`, React/JSX/TSX components, Ant Design (`antd`), Vite, CSS/SCSS styling, or any visual front-end deliverable. Not for back-end (FastAPI/ChromaDB), CLI tools, infra (Docker), or pure logic work."
---

# Web Design Engineer (compose-yml)

> Upstream: `ConardLi/garden-skills` v1.2.2 (MIT). Customized for this project.
> Original skill body preserved below; project-specific notes follow the body.

## Project context (read first)

This repo is a Docker-Compose stack with two visual surfaces worth knowing about:

| Path | Stack | Notes |
|---|---|---|
| `frontend/src/` | React 18 + Vite + Ant Design 5 + TypeScript | Live React app, hot-reload via Vite |
| Standalone HTML | Plain HTML/CSS/JS | Throwaway prototypes, demos, shareable artifacts |

### Design tokens already in use

- Theme: **Ant Design `theme.darkAlgorithm`** (see `frontend/src/main.jsx`)
- Primary color token: `colorPrimary: '#722ed1'` (purple) — keep this for primary CTAs; use AntD tokens (`colorBgContainer`, `colorBorder`, etc.) rather than hardcoded hex
- Border radius: `borderRadius: 8`
- Body gradient: `linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)` (in `index.css` `.app-container`)
- Font stack: `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif`

### Tech constraints to respect

- **In-app work** (`frontend/src/**/*.tsx|jsx|css`): use AntD components (`Button`, `Card`, `Layout`, `Table`, etc.) and AntD tokens. Do not introduce Tailwind, MUI, Chakra, or other competing UI libs without asking.
- **Standalone HTML artifacts**: any HTML/CSS/JS file written outside `frontend/src/` is a one-off deliverable. Use vanilla techniques, the recipe library in `references/style-recipes/`, and ship a single self-contained `index.html` unless told otherwise.
- Dark mode is the default. Light-mode work must be explicitly requested.

### Anti-cliché reminders (still apply, even with AntD)

- ❌ Inter as a free-standing heading font (AntD's default stack is fine)
- ❌ Purple→pink gradients on body backgrounds (the existing dark-blue gradient already covers this — don't add another)
- ❌ Emoji icons in shipped UI; use `@ant-design/icons` or SVG
- ❌ Centered hero with three feature cards as the default landing pattern — pick a layout that earns its space
- ✅ Strong type hierarchy, deliberate whitespace, real data shapes, working interactions

## Project workflow override

When the request is for the live app (`frontend/src/`), the upstream Step 0–4 still applies but **Step 5 (Verification)** simplifies:

- Render path is Vite dev server (`npm run dev`) — verify with the browser MCP (`cursor-ide-browser`) at the actual dev URL
- Production path is `frontend/Dockerfile` → nginx on `:8002` — verify build with `docker compose build frontend` if you touch `Dockerfile`/`nginx.conf`

When the request is for a **standalone artifact** (one-off HTML page, prototype, deck), fall back to the full upstream workflow below.

---

## Upstream skill body follows
...
## compose-yml: Quick-reference commands

When you need to verify or serve the work in this repo:

```bash
# Live app — Vite dev server on :5173 (default)
...
# Production preview via Docker
docker compose up -d frontend   # serves on :8002
```

For visual verification, use the `cursor-ide-browser` MCP to navigate to the dev URL after starting it.

## Auto-trigger rules for this project

This skill loads automatically when the request matches **any** of:

1. Mentions `frontend/`, `App.jsx`, `App.tsx`, `.jsx`, `.tsx`, React component names, or AntD components
2. Asks for a "page", "screen", "dashboard", "view", "modal", "form", "table", "chart", "prototype", or "mockup" against the browser
3. Asks to redesign / restyle / improve the existing UI
4. Provides a screenshot, Figma link, or visual reference and asks for a code recreation

Do **not** load for:

- FastAPI routes, ChromaDB queries, embedding model issues, Dockerfile / docker-compose changes, Python deps, `.env`, backend logs, performance, security — these belong to back-end or infra skills (or no skill).
- Pure logic / refactor / test writing that touches no visual layer, even if the file is in `frontend/src/`.
