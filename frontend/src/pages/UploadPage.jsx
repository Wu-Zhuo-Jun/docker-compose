...
function UploadPage() {
  ...
  const beforeUpload = (file) => {
    ...
    if (!isDocx) {
      message.error("只能上传 .docx 或 .txt 格式的 Word 文档！");
    }
    return isDocx;
  };
              ...
              {uploadResult.chunks && uploadResult.chunks.length > 0 && (
                <div style={{ marginTop: "16px" }}>
                  ...
                  {uploadResult.chunks.slice(0, 3).map((chunk, index) => (
                    <Card key={chunk.id} size="small" style={{ marginBottom: "8px", background: "rgba(0,0,0,0.3)" }}>
                      <Space>
                  ...
                  {uploadResult.chunks.length > 3 && <Text type="secondary">...还有 {uploadResult.chunks.length - 3} 个块</Text>}
                </div>
              )}
            </Card>
          </div>
